...
def generate_session_stack(data, exercises):
    ...
    if data["mode"] == "BM":
        ...
        if data["practice"] == "EP":
            session_exercises = get_emergency_tasks(exercises, count=3)
        elif data["practice"] == "CT":
            session_exercises = get_city_traffic_tasks(exercises, count=3)
    
    elif data["mode"] == "SM":
        session_timing_slots = surprise_mode_timing_stack(data)

        if data["practice"] == "EP":
            session_exercises =  get_emergency_tasks(exercises)
        elif data["practice"] == "CT":
            session_exercises = get_city_traffic_tasks(exercises)
